...
def process_marked_lines_with_return_flag(lines, markers, return_flags=[False, -1, -1]):
    """Run regexes against message's marked lines to strip quotations.
    ...
    quotation = re.search("(se*)+((t|f)+e*)+", markers)
    if quotation:
        # for group in quotation.regs:
        #     print(f"{group} : {lines[group[0] : group[1]]}")
        return_flags[:] = [True, quotation.start(1), quotation.end(1)]
...
def extract_with_quotation_from_plain(msg_body):
    """Extracts a non quoted message from provided plain text."""
    delimiter = get_delimiter(msg_body)
    msg_body = preprocess(msg_body, delimiter)
    # don't process too long messages
...
def extract_all_from_plain(msg_body, results=None):
    """Extracts a non quoted message from provided plain text."""
    if not results:
        results = []
    delimiter = get_delimiter(msg_body)
    ...
    lines, return_flags = process_marked_lines_with_return_flag(origin_lines, markers)
    # print(
    #     f"[process_marked_lines_with_return_flag] lines : {lines} return_flags : {return_flags}"
    # )
